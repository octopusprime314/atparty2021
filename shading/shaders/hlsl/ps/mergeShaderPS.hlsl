...
float4 main(float4 posH : SV_POSITION, float2 uv : UVOUT) : SV_Target
{
    ...
    float2 velocityVector = velocityTexture.Sample(bilinearWrap, uv).xy / 8.0;
    float2 texCoords      = uv;

    result += deferredTexture.Sample(bilinearWrap, texCoords) * 0.4;
    texCoords -= velocityVector;
    result += deferredTexture.Sample(bilinearWrap, texCoords) * 0.3;
    texCoords -= velocityVector;
    result += deferredTexture.Sample(bilinearWrap, texCoords) * 0.2;
    texCoords -= velocityVector;
