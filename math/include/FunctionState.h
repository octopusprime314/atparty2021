...
//                        [=](double t) -> Vector4 { return static_cast<Vector4>(force) * (powf(M_E,
//                        -4.0*t));}, 5, 3000);
...
class FunctionState
{
  ...
  public:
    FunctionState(std::function<void(Vector4)>  functionPointer,
                  std::function<Vector4(float)> equationLambda, int milliseconds,
    ...
    ~FunctionState() { _terminate = true; }

    Vector4 getVectorState() { return _currValue; }
