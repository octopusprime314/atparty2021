...
PipelineShader::PipelineShader(std::string shader, ComPtr<ID3D12Device> device, DXGI_FORMAT format)
{
    ...
    HRESULT result =
        D3DCompileFromFile(shaderString.c_str(), 0, D3D_COMPILE_STANDARD_FILE_INCLUDE, "VS",
                           "vs_5_0", 0, 0, compiledVS.GetAddressOf(), errorsVS.GetAddressOf());

    result = D3DCompileFromFile(shaderString.c_str(), 0, D3D_COMPILE_STANDARD_FILE_INCLUDE, "PS",
    ...
    CD3DX12_DESCRIPTOR_RANGE srvTableRange;
    CD3DX12_DESCRIPTOR_RANGE samplerTableRange;
    ...
    descRootSignature.Init(static_cast<UINT>(_resourceIndexes.size()), &RP[0], 0, nullptr,
    ...
    pso.VS = {reinterpret_cast<BYTE*>(compiledVS->GetBufferPointer()), compiledVS->GetBufferSize()};
    pso.PS = {reinterpret_cast<BYTE*>(compiledPS->GetBufferPointer()), compiledPS->GetBufferSize()};
...
ComPtr<ID3D12PipelineState> PipelineShader::getPSO() { return _psoState; }

ComPtr<ID3D12RootSignature> PipelineShader::getRootSignature() { return _rootSignature; }

std::map<std::string, UINT>& PipelineShader::getResourceBindings() { return _resourceIndexes; }
...
std::wstring PipelineShader::stringToLPCWSTR(const std::string& s)
{
    ...
    int          modLen  = MultiByteToWideChar(CP_ACP, 0, s.c_str(), slength, buf, len);
    std::wstring r(buf);
    delete[] buf;
    return r;
}
