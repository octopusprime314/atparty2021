...
inline UINT Align(UINT size, UINT alignment) { return (size + (alignment - 1)) & ~(alignment - 1); }
...
class ShaderRecord
{
  public:
    ...
    void CopyTo(void* dest) const
    {
        ...
        {
            memcpy(byteDest + m_ShaderIdentifier.size, m_LocalRootArguments.ptr,
                   m_LocalRootArguments.size);
        }
    }
...
class ShaderTable
{
  ...
  public:
    ...
    {
        uint8_t* mappedData;
        // We don't unmap this until the app closes. Keeping buffer mapped for the lifetime of the
        // resource is okay. We do not intend to read from this resource on the CPU.
    ...
    void Allocate(ID3D12Device* device, UINT bufferSize, LPCWSTR resourceName = nullptr)
    {
        ...
        device->CreateCommittedResource(&uploadHeapProperties, D3D12_HEAP_FLAG_NONE, &bufferDesc,
    ...
    ShaderTable(ID3D12Device* device, UINT numShaderRecords, UINT shaderRecordSize,
    ...
    void DebugPrint(std::unordered_map<void*, std::wstring> shaderIdToStringMap)
    {
        ...
        for (UINT i = 0; i < m_shaderRecords.size(); i++)
        {
            ...
            wstr << m_shaderRecords[i].m_ShaderIdentifier.size << L" + "
                 << m_shaderRecords[i].m_LocalRootArguments.size << L" bytes \n";
        }
        wstr << L"|--------------------------------------------------------------------\n";
        wstr << L"\n";
