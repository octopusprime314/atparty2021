/*
 * IOEventDistributor is part of the ReBoot distribution
...
struct TimeEvent
{
    ...
    TimeEvent(uint64_t time, Callback* pfnCallback) : time(time), pfnCallback(pfnCallback) {}
};
static bool operator<(TimeEvent left, TimeEvent right) { return left.time > right.time; }
